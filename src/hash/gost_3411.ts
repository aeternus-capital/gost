...
const addXor512 = (a: Uint8Array, b: Uint8Array): Uint8Array => {
    const c: Uint8Array = new Uint8Array(BLOCK_SIZE);
    for (let i = 0; i < BLOCK_SIZE; i++) c[i] = a[i] ^ b[i];

    return c;
};

const addMod512 = (a: Uint8Array, b: Uint8Array): Uint8Array => {
...
const P = (a: Uint8Array): Uint8Array => {
    const b: Uint8Array = new Uint8Array(BLOCK_SIZE);
    for (let i = 0; i < BLOCK_SIZE; i++) b[i] = a[PBOX[i]];

    return b;
}

const S = (a: Uint8Array): Uint8Array => {
    const b: Uint8Array = new Uint8Array(BLOCK_SIZE);
    for (let i = 0; i < BLOCK_SIZE; i++) b[i] = S_BOX[a[i]];

    return b;
}

const L = (a: Uint8Array): Uint8Array => {
    const b: Uint8Array = new Uint8Array(BLOCK_SIZE);

    for (let i = 0; i < 8; i++) {
        const parts: Uint32Array = new Uint32Array(2);
        const tmp: Uint8Array = a.slice(i * 8, i * 8 + 8).reverse();
        ...
        b[i * 8] = parts[0] >> 24;
        b[i * 8 + 1] = (parts[0] << 8) >> 24;
        b[i * 8 + 2] = (parts[0] << 16) >> 24;
        b[i * 8 + 3] = (parts[0] << 24) >> 24;
        b[i * 8 + 4] = parts[1] >> 24;
        b[i * 8 + 5] = (parts[1] << 8) >> 24;
        b[i * 8 + 6] = (parts[1] << 16) >> 24;
        b[i * 8 + 7] = (parts[1] << 24) >> 24;
    }

    return b;
}

const E = (a: Uint8Array, b: Uint8Array): Uint8Array => {
    let c: Uint8Array = addXor512(a, b);
    for (let i = 0; i < 12; i++) {
        a = L(P(S(addXor512(a, C[i]))));
        c = addXor512(L(P(S(c))), a);
    }

    return c;
}

const GN = (a: Uint8Array, b: Uint8Array, c: Uint8Array): Uint8Array => {
    return addXor512(addXor512(E(L(P(S(addXor512(b, a)))), c), b), c);
}
 ...
 * @example Basic usage
 * ```ts
 * import { GOST3411Hash } from '@aeternus/gost';
 *
 * const messages: Uint8Array = new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8]);
 * const hash256: Uint8Array = new GOST3411Hash(256).hash(message);
 * const hash512: Uint8Array = new GOST3411Hash(512).hash(message);
 * ```
 * @module
 */
class GOST3411Hash {
    private readonly size: 256 | 512;

    /**
     * GOST3411Hash constructor
     *
     * @param {number} size Output bits (possible values: 256 or 512)
     */
    constructor(size: 256 | 512) {
        this.size = size;
    }

    /**
     * Method hashes received message by GOST R 34.11–2012
     *
    ...
    public hash(message: Uint8Array): Uint8Array {
        ...
        let h: Uint8Array = new Uint8Array(64).fill(this.size === 512 ? 0 : 1);

        let blocks: number = 1;
        for (let i = message.length; i >= BLOCK_SIZE; i -= BLOCK_SIZE) {
            const pos: number = message.length - blocks * BLOCK_SIZE;

            h = GN(n, h, message.slice(pos, pos + BLOCK_SIZE));
            n = addMod512(n, new Uint8Array([0, 0, 2, 0]));
            sigma = addMod512(sigma, message.slice(pos, pos + BLOCK_SIZE));
            blocks++;
        }

        const paddedMsg: Uint8Array = new Uint8Array(BLOCK_SIZE);
